...
<div class="row">
    <div class="col-md-12">
        <div class="card card-primary">
            ...
            <form action="{{ route('bMasuk.update', $bMasuk->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <input type="hidden" name="oldPhoto" value="{{ $bMasuk->foto_nota }}">
                    <div class="form-group">
                        <label>Nama Suppliers</label>
                        <select name="supplier_id" id="supplier_id" class="form-control select2" required>
                            <option disabled selected>Pilih Suppliers</option>
                            @foreach ($supplierSelected as $item)
                            <option value="{{ $item->id }}"
                                {{ $bMasuk->supplier_id == $item->id ? 'selected' : null}}>
                                {{ $item->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="input-group date" id="tgl" data-target-input="nearest">
                            <input name="tgl_masuk" type="text" class="form-control datetimepicker-input" data-target="#tgl" value="{{ $bMasuk->tgl_masuk }}" required/>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                          <div class="custom-file">
                            ...
                            <label class="custom-file-label" for="foto_nota">{{ $bMasuk->foto_nota }}</label>
                          </div>
                        </div>
                        ...
                        @enderror
                        <br>
                        <img src="{{ asset('storage/nota_masuk/'.$bMasuk->foto_nota) }}" id="img-view" width="100px">
                      </div>
                    <div class="form-group">
                        <label>Keterangan</label>
                        <textarea class="form-control" name="keterangan" id="keterangan" cols="30" rows="3" required>{{ $bMasuk->keterangan }}</textarea>
                    </div>
                </div>
