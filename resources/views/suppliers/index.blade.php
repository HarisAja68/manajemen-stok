...
<div class="card">
    <div class="card-header">
        <a class="btn btn-primary btn-sm" href="{{ route('supplier.create') }}" role="button"><i
            class="fas fa-plus"></i>
        Tambah Data</a>
    </div>
    <!-- /.card-header -->
        ...
        <table id="tabel" class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$key+1}}</td>
                    <td>{{ $data->nama}}</td>
                    <td>{{ $data->nomer_tlpn}}</td>
                    <td>{{ $data->alamat }}</td>
                    <td>
                        <a href="{{ route('supplier.edit', $data->id) }}" class="btn btn-sm btn-warning"><i
                                class="fas fa-edit"> Edit</i></a>
                        <form class="d-inline" action="{{ route('supplier.destroy', $data->id) }}" role="alert"
                            method="POST">
                            @csrf
                            @method('DELETE')
